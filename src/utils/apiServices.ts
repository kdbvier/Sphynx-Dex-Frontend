...
async function getTokenInfoForChart(input: any, quoteAddress: any, pair: any, routerVersion: any, chainId = 56) {
  ...
  const quoteTokenPrice =
    chainId === ChainId.MAINNET ? await getTokenPrice(quoteAddress) : await getTokenPriceETH(quoteAddress)
...
async function getChartData(
  ...
  const quoteTokenPrice =
    chainId === ChainId.ETHEREUM ? await getTokenPriceETH(quoteAddress) : await getTokenPrice(quoteAddress)
